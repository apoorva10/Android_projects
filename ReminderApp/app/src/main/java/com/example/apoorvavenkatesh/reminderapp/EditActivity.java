...
public class EditActivity extends AppCompatActivity {
    ...
    List<Long> tempIds = remind.getTempIds();
...
//        timeLabel.setTypeface(lightFont);
        ...
        try {
            Alarm firstAlarm = remind.getAlarmById(getApplicationContext(), tempIds.get(0));
            hour = firstAlarm.getHour();
            minute = firstAlarm.getMinute();
            timeLabel.setText(setTime(hour, minute));
           remind.setTempName(firstAlarm.getPillName()) ;
        ...
        tempPill_name = remind.getTempName();
        editText.setText(tempPill_name);

        for(Long id: tempIds){
            try{
                int day = remind.getDayOfWeek(getApplicationContext(), id);
        ...
        View.OnClickListener setClickListener = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!remind.reminderExist(getApplicationContext(), pill_name)) {
                    Remind pill = new Remind();
                    pill.setReminderName(pill_name);
                    alarm.setHour(hour);
                    alarm.setMinute(minute);
                    alarm.setPillName(pill_name);
                    alarm.setDayOfWeek(dayOfWeekList);
                    pill.addAlarm(alarm);
                    long pillId = remind.addReminder(getApplicationContext() ,pill);
                    pill.setReminderId(pillId);
                    remind.addAlarm(getApplicationContext(), alarm, pill);
                } else { // If Pill already exists
                    Remind pill = remind.getReminderByName(getApplicationContext(), pill_name);
                    alarm.setHour(hour);
                    alarm.setMinute(minute);
                    alarm.setPillName(pill_name);
                    alarm.setDayOfWeek(dayOfWeekList);
                ...
                try {
                    List<Alarm> alarms = remind.getAlarmByPill(getApplicationContext(), pill_name);
                ...
                for(int i=0; i<7; i++) {
                    if (dayOfWeekList[i] && pill_name.length() != 0) {
                        ...
                        intent.putExtra("Reminder Name", pill_name);
                ...
                if(checkBoxCounter == 0 || pill_name.length() == 0)
                    Toast.makeText(getBaseContext(), "Please input a reminder or check at least one day!", Toast.LENGTH_SHORT).show();
                else { // Input form is completely filled out
                    for (long alarmID : tempIds) {
                        remind.deleteAlarm(getApplicationContext(), alarmID);
                    ...
                    try {
                        List<Alarm> tempTracker = remind.getAlarmByPill(getBaseContext(), tempPill_name);
                        if(tempTracker.size() == 0)
                            remind.deletePill(getBaseContext(), tempPill_name);
                    } catch (URISyntaxException e) {
                        e.printStackTrace();
                    }
        ...
        View.OnClickListener cancelClickListener = new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent returnHome = new Intent(getBaseContext(), ReminderActivity.class);
                startActivity(returnHome);
                finish();
            }
        };
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_delete) {
            ...
            Intent returnPillBox = new Intent(getBaseContext(), ReminderActivity.class);
    ...
    public void onBackPressed() {
        Intent returnPillBoxActivity = new Intent(getBaseContext(),ReminderActivity.class);
